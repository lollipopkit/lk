  ...
  8. 当前验收记录
      ...
      - [x] direct-call 容器参数 facts 现在保留同质 value/element 类型：参数推断不再把 `Map<_, Int>` / `List<Int>` 压成纯 `Any`，函数参数预绑定统一走 `apply_type_fact`，显式/推断参数都可把容器 value facts 注入函数体 builder；List lookup 只在已知长度和已知安全索引时传播 element fact，避免把越界/负数 `list.get` 误编成 typed value。新增 `direct_call_preserves_homogeneous_int_map_param_fact`、loop/compound/two-map 参数覆盖，并由 `test_list_get_out_of_bounds_yields_nil` 覆盖 list.get 语义边界；`cargo test -p lk-core vm::compiler::tests -- --nocapture` 通过。`coverage --runtime bench/workloads_business_algorithms.lk` checksum 全匹配、AOT entry native-lowerable、packed=8/8、`quickening_misses=15`、`quickening_sentinel_skips=0`；该步骤是通用函数边界 fact 架构补强，当前 workload 未形成宏观收益结论。
      - [x] 已知闭包调用后的 global reload 从“无捕获也重载所有 globals”收窄为只重载真实捕获的 global/register capture 和函数体静态写入的 globals；同时拆出 `stmt/globals.rs`、`stmt/traits.rs` 和 `builder_support.rs`，`stmt.rs=920`、`builder.rs=994`，均低于 1000 行警告线。新增 `known_closure_without_global_captures_does_not_reload_all_globals` 防止无全局捕获闭包调用重新引入批量 `LoadGlobal`，并保留 `top_level_closure_can_compound_assign_captured_global` 覆盖闭包写全局后的必要 reload。`coverage --runtime bench/workloads_business_algorithms.lk` checksum 全匹配、AOT entry native-lowerable、packed=8/8，总 ops 1290 -> 1029，entry packed ops 1165 -> 904，`LoadGlobal` 317 -> 56，global category 429 -> 168，`register_writes` 38172263 -> 38172002，`quickening_misses=15`、`quickening_sentinel_skips=0`；这是通用 global reload 架构修正，不按特定 workload 名称优化。
      - [x] global reload 收窄后的短验先发现 bench 脚本不会自动重建 `target/release/lk`，因此重新执行 `cargo build --release -p lk-cli` 后再跑 `RUNS=3 EXTRA_RUNS=3 bench/run_workload_bench.sh`：checksum 全匹配，VM geomean 3.675x vs Lua、AOT geomean 2.567x vs Lua；`gcd_batch` / `prime_trial_division` / `matrix_3x3_multiply` / `stock_max_profit` / `order_score_pipeline` 为 low-confidence 噪声。该短验确认 correctness 和覆盖统计稳定，但宏观 VM 验收仍未达标；同时补充 `unresolved_external_call_does_not_reload_toplevel_globals` 和 branching helper 的 direct-call map-param facts 回归测试，防止全局 reload 与容器 fact 边界倒退。
      - [x] top-level global slot coherence 后，已定义在本编译单元内的 top-level global 读优先使用本地 register slot，只在没有本地槽的外部符号上使用 `LoadGlobal`；这会保留 `DefineGlobal` 对外同步，但避免同一函数内的参数传递/表达式读取反复走 context hash lookup。改动前先拆出 `compiler/expr_operands.rs`，`expr.rs` 从 1013 行降到 863 行；新增 `toplevel_global_arguments_read_from_local_slots` 覆盖 call args 不再回读全局表。`coverage --runtime bench/workloads_business_algorithms.lk` checksum 全匹配、AOT entry native-lowerable、packed=8/8，总 ops 1029 -> 1007，entry packed ops 904 -> 882，`LoadGlobal` 56 -> 0，global category 168 -> 113，`heap_clones` 901253 -> 781253，`val_clones` 12576007 -> 12335891，`register_writes` 回到 38171980；`cargo check --all-features --workspace --all-targets`、`cargo test --workspace` 通过。重建 release 后短验 `RUNS=3 EXTRA_RUNS=3 bench/run_workload_bench.sh` checksum 全匹配，VM geomean 3.544x vs Lua、AOT geomean 2.269x vs Lua；仍未达到宏观验收，但这是跨类别的通用 VM 主路径改善。
      - [x] top-level loop global write flush 后，循环体内对已定义 top-level global 的写入以 local register 为 source of truth，range for / generic for / while / while-let 统一在循环出口一次性 `DefineGlobal`，避免热循环每轮同步 context hash table，同时保持 break/条件失败/while-let nil 退出后的外部可见性。新增 `toplevel_loop_global_write_flushes_once_after_range_loop` 验证 range loop body 不再包含 per-iteration `DefineGlobal`，新增 `toplevel_loop_global_writes_visible_after_all_loop_forms` 覆盖 range for、list for、while 和 break 后函数读取最终 global 值。`coverage --runtime bench/workloads_business_algorithms.lk` checksum 全匹配、AOT entry native-lowerable、packed=8/8，总 ops 1007 -> 994，entry packed ops 882 -> 869，global category 113 -> 100，`DefineGlobal` 111 -> 98，`val_clones` 12335891 -> 9704721，`immediate_clones` 11554638 -> 8923468，`heap_clones` 保持 781253，`quickening_misses=15`、`quickening_sentinel_skips=0`；`cargo fmt --all --check`、`cargo check --all-features --workspace --all-targets`、`cargo test --workspace`、`git diff --check` 均通过。重建 release 后短验 `RUNS=3 EXTRA_RUNS=3 bench/run_workload_bench.sh` checksum 全匹配，VM geomean 2.952x vs Lua，AOT geomean 0.753x vs Lua；这是跨循环形态的通用 VM 主路径改善，但仍未把 VM geomean 压到可视为完成的水平。
      - [x] `VmContext` method-dispatch helper 拆分后，`context.rs` 从 1149 行降到 959 行，新增 `context/core_methods.rs` 承载 `__lk_call_method` / named method fallback，满足大文件先拆分约束；随后 `VmContext::set` / `define_const` 对已经拥有的 `Val` 直接 move 进 map，只在 slot mirror 需要时由 `try_update_slot` clone，避免 context write path 的二次 value clone。`cargo check --all-features --workspace --all-targets`、`cargo test --workspace`、`cargo fmt --all --check`、`git diff --check` 均通过；`coverage --runtime bench/workloads_business_algorithms.lk` checksum 全匹配、AOT entry native-lowerable、packed=8/8，静态 ops 保持 994，`val_clones` 9704721 -> 9692127，`immediate_clones` 8923468 -> 8910885，`heap_clones` 781253 -> 781242，`register_writes` 保持 38171980。重建 release 后短验 `RUNS=3 EXTRA_RUNS=3 bench/run_workload_bench.sh` checksum 全匹配，VM geomean 2.948x vs Lua，AOT geomean 0.740x vs Lua；这是基础写入路径清理，宏观收益较小但无回归。
      - [x] Float register/value facts 已补齐到 VM compiler facts：`Type::Float`、Float 常量、同质 Float List/Map、direct-call Float 参数 inference、`LoadLocal`/`Move`/`StoreLocal` 传播都会进入 `float_regs`，并驱动 `AddFloat` / `SubFloat` / `MulFloat` / `ModFloat` 选择；同时先拆出 `builder_facts.rs`，`builder.rs=597`、`builder_facts.rs=451`，避免在接近 1000 行的大文件上继续堆逻辑。新增 `homogeneous_float_list_index_feeds_typed_arithmetic`、`homogeneous_float_map_get_feeds_typed_arithmetic`、`direct_call_float_param_fact_feeds_typed_arithmetic`，并补充 `pricing_helper_keeps_two_map_param_value_facts` 防止 Map<Int> 参数链退回 generic multiply。`cargo fmt --all --check`、`cargo check --all-features --workspace --all-targets`、`cargo test --workspace`、`git diff --check` 均通过；`coverage --runtime bench/workloads_business_algorithms.lk` checksum 全匹配、AOT entry native-lowerable、packed=8/8，静态 ops/typed gate/runtime metrics 与上一步一致，`val_clones=9692127`、`quickening_misses=15`、`quickening_sentinel_skips=0`。该步骤是通用 typed fact 架构补齐，当前 business workload 未触发宏观收益，不作为验收完成。
      - [x] direct-call 参数推断现在识别已常量折叠的 `Expr::Val(Val::List/Map)`，并保留同质 container value/element 类型；此前只识别 AST 级 `Expr::List/Map`，完整程序里的常量 Map 可能已经折叠成 `Val::Map`，导致函数边界 `Map<Int>` fact 丢失并退回 generic `Access` / `Mul`。新增 `direct_call_infers_folded_map_value_fact` 覆盖折叠 Map 作为 direct-call 参数仍能在 callee 内发射 `MapGetDynamic` 和 typed add。`cargo fmt --all --check`、`cargo check --all-features --workspace --all-targets`、`cargo test --workspace`、`git diff --check` 均通过；`coverage --runtime bench/workloads_business_algorithms.lk` checksum 全匹配、AOT entry native-lowerable、packed=8/8，`cart_line_total` 从 `Access=2` / `Mul=2` 改为 `MapGetDynamic=2` / `MulInt=2`，typed gate `MapGetDynamic=6 -> 9`、`MulInt=30 -> 32`、`SubInt=29 -> 30`，runtime clone/write metrics 保持 `val_clones=9692127`、`register_writes=38171980`、`quickening_misses=15`、`quickening_sentinel_skips=0`。重建 release 后短验 `RUNS=3 EXTRA_RUNS=3 bench/run_workload_bench.sh` checksum 全匹配，VM geomean 2.927x vs Lua、AOT geomean 0.754x vs Lua；`matrix_3x3_multiply` / `route_permission_check` 为 low-confidence 噪声，因此记录为通用 folded-constant fact 修复和 typed dispatch 覆盖改善，不作为宏观验收完成。
      - [x] stdlib global method return fact 扩展到 `os.clock()` 的 Float 返回，并让 direct-call 参数推断识别零参 `os.clock()` / `os.epoch()` / `os.time()` 调用结果；这样跨 helper 函数传递的时间戳参数也能进入 Float/Int register facts，不需要针对某个 workload 名称写 lowering。测试侧先把 stdlib return fact 用例拆到 `compiler/tests/stdlib_return_facts.rs`，使 `general_optimizations.rs=891`、新文件 `stdlib_return_facts.rs=204`，避免继续把接近 1000 行的测试文件做大；新增 `known_global_clock_return_feeds_typed_float_arithmetic`、`direct_call_infers_global_clock_float_arguments` 覆盖直接表达式和 direct-call 参数两条路径。`cargo test --workspace`、`cargo build --release -p lk-cli` 通过；`coverage --runtime bench/workloads_business_algorithms.lk` checksum 全匹配、AOT entry native-lowerable、packed=8/8，`emit` closure 的 elapsed ms 计算从 generic multiply 变为 `MulFloat=1`，typed gate `CmpI=9`，静态 `code32_words=1682`，runtime metrics 保持 `val_clones=9692127`、`immediate_clones=8910885`、`heap_clones=781242`、`register_writes=38171980`、`quickening_misses=15`、`quickening_sentinel_skips=0`。重建 release 后短验 `RUNS=3 EXTRA_RUNS=3 bench/run_workload_bench.sh` checksum 全匹配，VM geomean 2.865x vs Lua、AOT geomean 0.747x vs Lua；`prime_trial_division` / `stock_max_profit` / `order_score_pipeline` / `route_permission_check` 为 low-confidence 噪声，因此记录为通用 stdlib return fact 与 typed dispatch 覆盖改善，不作为宏观验收完成。
      - [x] `Cmp*Imm` 现在真正支持 i16 immediate：先拆出 `bc32/compare.rs`，使 `bc32.rs=932`，再把非 i8 的 compare immediate 编码为 BC32 ext op，并在 packed hot slot 中融合 `CmpImm + BoolBranch`，避免 typed compare 重新写临时 Bool register。该改动覆盖所有 `== != < <= > >=` 的 i16 阈值比较，不绑定 workload 名称；新增 `wide_int_immediate_comparison_lowers_to_cmp_imm` 和 `cmp_imm_roundtrips_i16_immediate` 覆盖 compiler lowering 与 BC32 roundtrip。`cargo fmt --all --check`、`cargo check --all-features --workspace --all-targets`、`cargo test --workspace`、`git diff --check` 均通过；`coverage --runtime bench/workloads_business_algorithms.lk` checksum 全匹配、AOT entry native-lowerable、packed=8/8，`fraud_score` 中 `CmpGt=2` 变为 `CmpGtImm=2`，`code32_words=1684`，runtime `register_writes` 38171980 -> 36252889，`val_clones=9692127`、`quickening_misses=15`、`quickening_sentinel_skips=0` 保持稳定。重建 release 后短验 `RUNS=3 EXTRA_RUNS=3 bench/run_workload_bench.sh` checksum 全匹配，VM geomean 2.815x vs Lua、AOT geomean 0.746x vs Lua；`prime_trial_division` / `matrix_3x3_multiply` / `stock_max_profit` / `order_score_pipeline` 为 low-confidence 噪声，因此记录为通用 compare-dispatch/register-write 改善，不作为宏观验收完成。
      - [x] opcode 解释器路径补齐 `Cmp*Imm + JmpFalse/BoolBranch` 直接融合，与 packed hot 已有的 immediate compare branch fusion 对齐；执行相邻分支时直接根据 immediate compare 结果跳转，跳过临时 Bool register 写入。实现前 `opcode.rs` 一度越过 1000 行警告线，因此先把逻辑收敛到 `opcode/compare_ops.rs`，最终 `opcode.rs=928`、`compare_ops.rs=444`、`quickening/tests.rs=369`，均低于 1000 行警告线；新增 `opcode_compare_imm_branch_fusion_skips_temp_bool_register_write` 验证解释器路径的 register write 数不回退。`cargo test -p lk-core opcode_compare_imm_branch_fusion_skips_temp_bool_register_write -- --nocapture`、`cargo fmt --all --check`、`cargo check --all-features --workspace --all-targets`、`cargo test --workspace`、`git diff --check` 均通过。该步骤是 opcode interpreter parity，packed/release bench 预期不变，因此不作为新的宏观 bench 结论。
      - [x] compound assignment 的 RHS call 防御性旧值保存现在走保守 effect guard：只有 RHS 中所有 call 递归可证明不会重绑定目标 binding 时，`x += f(...)` 才直接读目标 slot；递归/未知用户闭包、可能写目标的 closure 仍保留 `LoadLocal` 旧值保护，维持 `x += bump()` 使用 call 前 LHS 的语义。新增 `compound_assign_known_leaf_call_reads_target_slot_directly` 和 `compound_assign_call_that_writes_target_preserves_old_lhs_value` 覆盖优化路径与副作用保护路径。`coverage --runtime bench/workloads_business_algorithms.lk` checksum 全匹配、AOT entry native-lowerable、packed=8/8，总 ops 994 -> 981，entry packed ops 869 -> 856，local category 69 -> 56，`code32_words` 1684 -> 1658，runtime `register_writes` 36252889 -> 35852589，`val_clones` 9692127 -> 9291814，`heap_clones=781242`、`quickening_misses=15`、`quickening_sentinel_skips=0` 保持稳定；`cargo fmt --all --check`、`cargo check --all-features --workspace --all-targets`、`cargo test --workspace` 均通过。重建 release 后短验 `RUNS=3 EXTRA_RUNS=3 bench/run_workload_bench.sh` checksum 全匹配，VM geomean 2.643x vs Lua、AOT geomean 0.754x vs Lua；`prime_trial_division` / `matrix_3x3_multiply` / `route_permission_check` / `fraud_rule_scoring` 为 low-confidence 噪声，因此记录为通用 effect-aware register-write/clone 改善，不作为最终验收完成。
      - [x] expression-only known call inline 现在允许无前缀语句的纯返回表达式直接内联，避免 `return add_one(41)` 这类简单 direct call 仍生成 call-window moves；同时多语句 closure block 的 const-env 注册改为和正常 closure 编译一致的 `Stmt::Block`，修复 `let f = |x| { let y = x + 1; y }; return f(4);` 被错误折叠为 `Nil` 的回归。新增 `expression_only_known_call_inlines_without_call_window_moves`，既有 `multi_statement_closure_block_returns_final_expression` 覆盖正确性边界；该改动使 business workload 静态 ops 从 981 降到 977、entry packed ops 从 856 降到 852、`code32_words` 从 1658 降到 1647，但 runtime `val_clones` 回升到 9561810、`register_writes` 回升到 35942589，说明这是通用 inliner/const-eval 正确性修复与小幅静态压缩，不作为宏观收益完成。
      - [x] packed hot cache 新增 `MoveCall` slot：当连续 `Move` 只填充后续 `Call` / `CallClosureExact` / `CallExact` 的参数窗口时，从第一个 `Move` 构建 fused hot slot，缓存 moves 与 call operands，一次 hot dispatch 完成参数复制和统一 `run_call_packed` 调用；这保留所有 call IC、return layout、closure/native/default 参数语义，不对特定 workload 名称或函数体做特殊处理。新增 `packed_hot_slot_fuses_moves_into_closure_exact_call_window` 覆盖 BC32 decoded span 与 ext call next-PC。A/B 短验显示禁用该 fusion 后 VM geomean 约 2.961x，启用后两轮为 2.806x / 2.782x，因此保留该通用 dispatch fusion；但相对上一轮 2.643x 未形成宏观提升，当前仍不能算验收完成。
      - [x] 2026-05-20 本轮验证：`cargo fmt --all --check`、`cargo check --all-features --workspace --all-targets`、`cargo test --workspace`、`git diff --check`、`cargo build --release -p lk-cli` 均通过；`coverage --runtime bench/workloads_business_algorithms.lk` checksum 全匹配、AOT entry native-lowerable、packed=8/8，`quickening_misses=15`、`quickening_sentinel_skips=0`。重建 release 后短验 `RUNS=3 EXTRA_RUNS=3 bench/run_workload_bench.sh` 两轮 checksum 全匹配，VM geomean 2.806x / 2.782x vs Lua，AOT geomean 0.860x / 0.852x vs Lua；该结果低于上一轮 effect-aware compound assignment 记录，必须继续优化剩余 generic dispatch / clone / register-write，而不是视为阶段完成。
