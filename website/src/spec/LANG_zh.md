...
- 重复语法支持 Rust 风格的 `$( ... )*`、`$( ... )+` 与 `$( ... )?`，并支持可选分隔符，例如 `$( $x:expr ),*`。Nested repetition 会在 template substitution 中保留 capture shape，包括空 `*`/`?` capture 与可选 nested repetition。LK 会拒绝 zero-width repetition pattern 和宏调用中的尾随分隔符，生成 nested separator 时不会产生尾随分隔符，并在解析期校验重复的 matcher 绑定名以及 matcher/template 重复深度不匹配。
- 宏展开发生在普通解析与类型检查之前。捕获的标识符按调用点解析；宏模板中新引入的局部绑定位置会 freshen，避免常见命名冲突，包括 `let`/`const` 解构、短声明 `name := value`、`for`/`if let`/`while let`、`match` arm pattern 与 guard、`select case value <- recv(...)` 绑定，同时不会 freshen 生成的 `recv`/`send` 操作名，以及位置/具名/默认值函数参数和闭包参数。生成的 semantic name 与 definition-name 位置（如成员访问字段 `object.item`、map literal 裸 key、struct literal 字段名、named argument key、`fn`/`struct`/`trait`/`type` 定义名，以及 struct declaration 字段名）会被保留，不会被当作局部绑定引用 freshen。生成的 type-reference 位置（如 `let`/`const` 类型注解、函数参数与返回类型、type alias target，以及 `impl Trait for Type` 头部）也会被保留。
- 使用 `export macro_rules! name { ... }` 或 `export { internal as public };` 从文件/package 导出宏。普通 `macro_rules!` 定义只在定义所在文件/模块内可用，对外部宏导入保持私有。
- 已导出的宏可以通过 `$crate::helper!()` 调用定义所在文件/package 内的私有 helper 宏，也可以生成 `$crate::helper()` 这类按宏定义模块解析的运行时 item 引用。导入的 file/package 宏会为该 anchor 注入内部运行时 namespace import；同文件 anchor 会改写为本地 item 引用。
...
- 宏展开错误会包含逐条 rule 的 mismatch notes，并附带 expansion stack 展示嵌套宏调用链；由宏生成 token 触发的 parse error 与 strict type-check diagnostic 会包含 macro origin stack。LSP type diagnostics 使用同一套 token origin。Parse 后 AST macro 的输入位置也会暴露 hover 文本和 generated item 的 document-symbol 条目。同文件以及导入的 file/package 宏调用支持 goto-definition 到对应 `macro_rules!` 定义。普通引用也可以通过 expanded token span 跳转到 `macro_rules!` 生成的 item 定义，通过 expanded token span 与 AST generated item origin 跳转到 manifest-backed proc/AST 生成的 item 定义，并通过 AST generated member origin 跳转到 `value`、`show`、`user.profile.id`、`user.profile.render`、`seed`、`current`、`kind`、`Alias`、`Boxed`、`Reader`、`read`、`User` 这类 external 或内置生成成员、field-access expression、call-callee reference、variable reference、assignment target reference、binding origin、semantic-name origin、generated declaration label 与 type reference；更完整的 generated-reference navigation 仍在计划中。
...
- `Lk.toml` 可以用 `[macros.derive.NAME]`、`[macros.attribute.NAME]` 与 `[macros.function_like.NAME]` 表声明进程型 provider。每个 provider 使用 `command`、可选 `args`、可选 `timeout_ms` 与可选 `max_output_bytes`；目前 derive、attribute 与 function-like provider 都已接入解析器。依赖中的 provider 只有在当前 package 通过 `[macros].trusted_dependencies` 显式信任该依赖后才会被发现；受信任依赖的 function-like provider 通过 `package::name!()` 调用。
- Procedural macro dependency metadata 会参与缓存失效。LK 会 fingerprint provider 报告的 `path`/`digest` 条目以及可读取依赖文件的当前状态；直接 native 执行会在缓存可执行文件旁写入 `.proc-macro-deps.json` sidecar，LSP workspace cache 会在依赖 fingerprint 过期时丢弃预加载分析，也会在保存依赖路径时通过反向依赖图失效受影响的缓存 LK 文件。
- Procedural macro 输出 token 会保留 provider 提供的 span，用于后续 parse diagnostics；缺失输出 span 时会回退到宏调用或 attribute span。展开后的 token stream 还会暴露逐 token origin，用于区分 call-site capture、macro definition token、`$crate` anchor 与 function-like proc macro output。Parse 后的 derive/attribute/cfg expansion 也会记录 item-level AST macro origin、结构化 generated-item origin、external generated function/trait/impl member origin、`expr user.profile.id` 这类 generated static field-access expression origin、`call helper` 与 `call user.profile.render` 这类 generated call-callee reference origin、`ref seed` 这类 generated variable reference origin、`assign_ref current` 与 `compound_assign_ref current` 这类 generated assignment target reference origin、`binding self` 与 `binding current` 这类 generated binding origin、`struct_field id`、map literal 和 map/object destructuring pattern 中的 `map_key kind`，以及 `named_arg current` 这类 generated semantic-name origin、来自 item signature、field declaration、alias、impl target、statement-local type annotation、struct literal，以及 top-level item 或 statement/block body 内生成声明的 `type_ref User` 这类 generated type reference origin、`type Alias`、`struct Boxed`、`trait Reader`、`fn read` 这类 generated declaration label，以及 `fn show`、`expr self.field` 这类内置 derive generated member/expression origin，并通过 `lk macro expand --origins`、LSP hover 和 LSP document symbols 暴露。
- 使用 `lk macro expand <file> --trace --deps --origins` 可以查看展开后的 token stream、token 展开 trace、收集到的 procedural macro dependencies、token origin JSON、AST macro origin JSON、结构化 AST generated-item/member origin JSON，以及 parse 后的 AST derive/attribute 展开结果。
- 示例：
...
## 包
- `Lk.toml` 定义 `[package]`、`[dependencies]`、`[workspace]`、`[workspace.dependencies]`、可选的 `[registry]` 发布元数据，以及可选的 `[macros.*]` procedural macro provider 表与用于显式信任依赖宏 provider 的 `[macros].trusted_dependencies`。`lk pkg check` 会在共享宏包前校验 macro provider 名称、路径型 provider command、信任条目和 package graph metadata。
- 字符串依赖默认来自 GitHub，例如：`util = "owner/repo"`。Registry 依赖使用 `{ version = "x.y.z" }`，默认通过 `[registry].url` 解析，也可以在依赖上用 `registry = "https://..."` 覆盖。
- `Lk.lock` 保存具体 revision 的已抓取 Git 源码，包括从 `GET [registry].url/api/v1/packages/<name>/<version>` 响应解析出的 registry version。
- `lk pkg publish --dry-run` 会校验 `[package].version`、`[registry].url`、include globs、依赖 version/revision metadata 与 macro provider 列表，然后打印带有 `integrity.sha256` canonical payload digest 的 registry publish manifest JSON。`lk pkg publish` 会在上传前重新校验该 digest，并使用 `LK_REGISTRY_PUBLISH_TOKEN`、`LK_REGISTRY_TOKEN` 或 `LK_PUBLISH_TOKEN`，把同一份 manifest 作为 authenticated JSON 上传到 `[registry].url/api/v1/packages`。Registry version response 与 index entry 可以携带 `publish_manifest`；LK 会校验其 package、version、registry URL 与 integrity digest 后才信任该版本。
