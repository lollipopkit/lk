...
fn write_path(args: NativeArgs<'_>, runtime: &mut NativeRuntime<'_>) -> Result<RuntimeVal> {
    ...
    let content = runtime_string_arg(&values[1], runtime.heap(), "file.write content")?;
    std::fs::write(path.as_ref(), content.as_bytes())
        .map_err(|err| anyhow!("failed to write file '{}': {err}", path))?;
...
fn append(args: NativeArgs<'_>, runtime: &mut NativeRuntime<'_>) -> Result<RuntimeVal> {
    ...
    let content = runtime_string_arg(&values[1], runtime.heap(), "file.append content")?;
