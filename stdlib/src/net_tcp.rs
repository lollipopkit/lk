...
use crate::{
    resource::{payload_int, payload_resource, payload_string, resource_arg, resource_value, task_value},
...
fn write(args: NativeArgs<'_>, runtime: &mut NativeRuntime<'_>) -> Result<RuntimeVal> {
    ...
    let data = runtime_string_arg(&values[1], runtime.heap(), "tcp.write data")?;
...
fn read_task(args: NativeArgs<'_>, runtime: &mut NativeRuntime<'_>) -> Result<RuntimeVal> {
    let (stream, max) = read_args(args, runtime)?;
    spawn_task(runtime, async move { Ok(payload_string(read_stream(stream, max)?)) })
}
...
fn write_task(args: NativeArgs<'_>, runtime: &mut NativeRuntime<'_>) -> Result<RuntimeVal> {
    ...
    let data = runtime_string_arg(&values[1], runtime.heap(), "tcp.write_task data")?.to_string();
    spawn_task(runtime, async move {
        let written = write_stream(stream, data.as_bytes())?;
...
fn read_stream(mut stream: TcpStream, max: usize) -> Result<String> {
    let mut buffer = vec![0u8; max];
    let read = stream.read(&mut buffer).map_err(|err| anyhow!("tcp read: {err}"))?;
    buffer.truncate(read);
    String::from_utf8(buffer).map_err(|_| anyhow!("tcp read data is not valid UTF-8"))
}
