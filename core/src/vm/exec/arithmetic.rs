...
#[inline]
fn compare_int_values(opcode: Opcode, lhs: i64, rhs: i64) -> Result<bool> {
    Ok(match opcode {
        Opcode::CmpLtInt => lhs < rhs,
        Opcode::CmpLeInt => lhs <= rhs,
        Opcode::CmpGtInt => lhs > rhs,
        Opcode::CmpGeInt => lhs >= rhs,
        other => bail!("Opcode {:?} is not an integer ordering comparison", other),
    })
}

#[inline]
fn compare_float_values(opcode: Opcode, lhs: f64, rhs: f64) -> Result<bool> {
    Ok(match opcode {
        Opcode::CmpLtInt => lhs < rhs,
        Opcode::CmpLeInt => lhs <= rhs,
        Opcode::CmpGtInt => lhs > rhs,
        Opcode::CmpGeInt => lhs >= rhs,
        other => bail!("Opcode {:?} is not a numeric ordering comparison", other),
    })
}

#[inline]
fn compare_string_values(opcode: Opcode, lhs: &str, rhs: &str) -> Result<bool> {
    Ok(match opcode {
        Opcode::CmpLtInt => lhs < rhs,
        Opcode::CmpLeInt => lhs <= rhs,
        Opcode::CmpGtInt => lhs > rhs,
        Opcode::CmpGeInt => lhs >= rhs,
        other => bail!("Opcode {:?} is not a string ordering comparison", other),
    })
}
...
impl Executor {
    ...
    #[cold]
    pub(super) fn dynamic_sub(&mut self, instr: Instr) -> Result<()> {
        let (dst, lhs, rhs) = self.stack_abc_indices(instr)?;
        if let Some(value) = {
            let lhs = &self.state.stack[lhs];
            let rhs = &self.state.stack[rhs];
    ...
    #[inline]
    #[cold]
    pub(super) fn number_compare_value(&self, opcode: Opcode, lhs_reg: u8, rhs_reg: u8) -> Result<bool> {
