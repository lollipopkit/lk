...
impl Compiler {
    ...
    fn lower_range_pattern_condition(
        ...
        let start = self.lower_readonly_operand(start)?;
        let end = self.lower_readonly_operand(end)?;
    ...
    fn lower_guard_condition(&mut self, pattern_condition: u16, guard: &crate::expr::Expr) -> Result<u16> {
        ...
        let guard = self.lower_readonly_operand(guard)?;
    ...
    fn lower_pattern_literal(&mut self, literal: &LiteralVal) -> Result<u16> {
        match literal {
            ...
            other => bail!(
                "Compiler cannot lower pattern literal with AST value kind {}",
