...
pub(super) fn collect_expr_free_vars(expr: &Expr, bound: &mut HashSet<String>, free: &mut Vec<String>) {
    match expr {
        ...
        Expr::Unary(_, inner) | Expr::Paren(inner) | Expr::Cast(inner, _) => collect_expr_free_vars(inner, bound, free),
...
pub(super) fn collect_expr_closure_captures(expr: &Expr, out: &mut Vec<String>) {
    match expr {
        ...
        Expr::Paren(inner) | Expr::Unary(_, inner) | Expr::Cast(inner, _) => collect_expr_closure_captures(inner, out),
        Expr::Bin(lhs, _, rhs)
        | Expr::And(lhs, rhs)
        | Expr::Or(lhs, rhs)
        | Expr::NullishCoalescing(lhs, rhs)
