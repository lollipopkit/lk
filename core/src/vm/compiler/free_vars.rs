...
pub(super) fn collect_expr_free_vars(expr: &Expr, bound: &mut HashSet<String>, free: &mut Vec<String>) {
    match expr {
        ...
        Expr::Unary(_, inner) | Expr::Paren(inner) | Expr::Cast(inner, _) | Expr::Unsafe(inner) => {
...
fn collect_stmt_free_vars(statements: &[Box<Stmt>], bound: &mut HashSet<String>, free: &mut Vec<String>) {
    for stmt in statements {
        match stmt.as_ref() {
            Stmt::Attributed { item, .. } | Stmt::Defer { body: item, .. } => collect_single_stmt_free_vars(item, bound, free),
...
pub(super) fn collect_expr_closure_captures(expr: &Expr, out: &mut Vec<String>) {
    match expr {
        ...
        Expr::Paren(inner) | Expr::Unary(_, inner) | Expr::Cast(inner, _) | Expr::Unsafe(inner) => {
            collect_expr_closure_captures(inner, out)
        }
        Expr::Bin(lhs, _, rhs)
        | Expr::And(lhs, rhs)
        | Expr::Or(lhs, rhs)
