    ...
    F: FnOnce(i64, i64) -> i64,
{
    ...
    if let Some((x, y)) = read_int_pair(regs, consts, a, b) {
        assign_reg_with_metrics(regs, dst_idx, Val::Int(op(x, y)), collect_metrics);
        Ok(())
    } else {
        ...
        let value = fallback.eval_vals_with_metrics(lhs, rhs, collect_metrics)?;
        assign_reg_with_metrics(regs, dst_idx, value, collect_metrics);
        Ok(())
    }
}
    ...
    F: FnOnce(f64, f64) -> f64,
{
    ...
    if let Some((x, y)) = read_float_pair(regs, consts, a, b) {
        assign_reg_with_metrics(regs, dst_idx, Val::Float(op(x, y)), collect_metrics);
        Ok(())
    } else {
        let lhs = rk_read(regs, consts, a);
        let rhs = rk_read(regs, consts, b);
    ...
    F: FnOnce(i64, i64) -> i64,
{
    ...
    if let Val::Int(x) = aval {
        assign_reg_with_metrics(regs, dst_idx, Val::Int(op(*x, imm as i64)), collect_metrics);
        return Ok(());
    }
    ...
    let value = fallback.eval_vals_with_metrics(aval, &imm_val, collect_metrics)?;
    assign_reg_with_metrics(regs, dst_idx, value, collect_metrics);
    Ok(())
}
...
pub(super) fn cmp_ord_imm(
    ...
    match aval {
        Val::Int(x) => {
            assign_reg_with_metrics(regs, dst_idx, Val::Bool(int_cmp(*x, imm_i64)), collect_metrics);
            Ok(())
        }
        Val::Float(x) => {
            assign_reg_with_metrics(regs, dst_idx, Val::Bool(float_cmp(*x, imm_i64 as f64)), collect_metrics);
            Ok(())
        }
        _ => {
            ...
            assign_reg_with_metrics(regs, dst_idx, Val::Bool(res), collect_metrics);
            Ok(())
        }
    }
}
...
pub(super) fn cmp_eq_imm(
    ...
    match aval {
        ...
        Val::Float(x) => {
            assign_reg_with_metrics(regs, dst_idx, Val::Bool(*x == imm_i64 as f64), collect_metrics);
            Ok(())
        }
        _ => {
            let imm_val = Val::Int(imm_i64);
