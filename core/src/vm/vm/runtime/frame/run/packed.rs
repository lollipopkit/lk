...
use crate::vm::{
    VmBc32FallbackMetric, VmCallMetric, copy_container_value_for_register_with_metrics,
    copy_value_for_register_with_metrics, record_bc32_fallback_op_known_enabled,
    record_bc32_fallback_reason_known_enabled, record_call_op_known_enabled, record_opcode_step_known_enabled,
    record_quickening_build_attempt_known_enabled, record_quickening_build_success_known_enabled,
    record_quickening_hit_known_enabled, record_quickening_miss_known_enabled,
    record_quickening_sentinel_skip_known_enabled, restore_register_value, take_register_value,
};
...
#[inline(always)]
fn assign_packed_const_with_metrics(regs: &mut [Val], func: &Function, dst: u16, kidx: u16, collect_metrics: bool) {
    assign_reg_const_copy_with_metrics(regs, dst as usize, &func.consts[kidx as usize], collect_metrics);
}
...
pub(super) fn run_packed_code(runtime: &mut FrameRuntimeView<'_, '_>) -> Result<Option<Val>> {
    ...
    while pc < code32.len() {
        ...
        let word = code32[pc];
        // FAST PATH: try hot slot first — avoids tag extraction + extension-word
        // skip logic for the common case where a quickened slot exists.
        if let Some(entry) = packed_hot.get(pc).and_then(|slot| slot.as_ref()) {
            match entry {
                PackedHotEntry::Slot(slot) if slot.word == word => {
                    ...
                    if let Some((f_reg, base, argc, retc, call_kind)) = hot_call_operands(&slot.kind) {
                        ...
                        if let PackedHotKind::MoveCall { moves, .. } = &slot.kind {
                            assign_move_call_args(regs, f, pc, moves, collect_metrics);
                        }
                    ...
                    // Prefetch next packed_hot entry to reduce cache miss on dispatch.
                    // Most slots use slot.next_pc; exec_hot_slot overrides it for branches.
                    #[cfg(target_arch = "x86_64")]
                    unsafe {
                        let ptr = packed_hot.as_ptr().add(slot.next_pc) as *const u8;
                ...
                PackedHotEntry::Miss(last_word) if *last_word == word => {
                    ...
                    // Fall through to tag checks + cold decode below.
                }
                _ => {} // No hot entry or mismatched word — fall through
            }
        }
        // SLOW PATH: tag checks + extension word logic, only reached on hot-slot miss
        ...
        // Try to build a hot slot (skipping sentinel-miss re-check — build_hot_slot
        // returns None for unquickenable words anyway, so the redundant attempt is
        // harmless and removes one packed_hot re-read from every cold-path iteration).
        {
        ...
        if let Some(next_pc) = try_exec_math_op(&op, regs, f, next_pc_default, collect_metrics)? {
            pc = next_pc;
            continue;
        }
        ...
        match op {
            ...
            Op::ListSlice { dst, src, start } => {
                ...
                if start_idx <= 0 {
                    let value = copy_container_value_for_register_with_metrics(&regs[src as usize], collect_metrics);
