...
use super::{RuntimeCallable32, RuntimeExport32, RuntimeModuleState32};

#[derive(Clone, Debug, Default, PartialEq, Eq)]
pub struct GcRoots32 {
    refs: Vec<HeapRef>,
}
...
impl RuntimeModuleState32 {
    pub fn gc_roots<'a>(&self, extra_roots: impl IntoIterator<Item = &'a RuntimeVal>) -> GcRoots32 {
        let active_stack_end = self.stack_top.min(self.stack.len());
        let extra_roots = extra_roots.into_iter();
        let mut roots = GcRoots32::with_capacity(self.globals.len() + active_stack_end + extra_roots.size_hint().0);
...
mod tests {
    ...
    #[test]
    fn collect_runtime_export32_keeps_export_value_and_state_roots() {
