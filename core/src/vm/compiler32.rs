...
impl Compiler32 {
    ...
    fn alloc_regs(&mut self, count: usize) -> Result<u16> {
    ...
    #[inline]
    fn emit(&mut self, instr: Instr32) {
        self.function.code.push(instr);
    }

    fn emit_move(&mut self, dst: u16, src: u16, context: &str) -> Result<()> {
        self.emit(Instr32::abc(
            Opcode32::Move,
    ...
    fn emit_test_placeholder(&mut self, condition: u16) -> Result<usize> {
    ...
    fn emit_raise(&mut self, message: &str) -> Result<()> {
    ...
    fn patch_test_false_jump(&mut self, pc: usize, target: usize) -> Result<()> {
        self.patch_test_jump(pc, target, 1)
    }

    fn patch_test_true_jump(&mut self, pc: usize, target: usize) -> Result<()> {
    ...
    fn patch_jmp(&mut self, pc: usize, target: usize) -> Result<()> {
        let instr = *self
            .function
            .code
            .get(pc)
    ...
    fn push_int(&mut self, value: i64) -> Result<u16> {
        self.function.consts.push_int(value)
    }

    fn push_float(&mut self, value: f64) -> Result<u16> {
        self.function.consts.push_float(value)
    }

    fn push_string(&mut self, value: &str) -> Result<u16> {
        self.function.consts.push_string(value)
    }

    fn push_heap_value(&mut self, value: ConstHeapValue32) -> Result<u16> {
        self.function.consts.push_heap_value(value)
    }

    fn emit_return(&mut self, base: u16) -> Result<()> {
        self.emit(Instr32::abc(Opcode32::Return, checked_u8("return base", base)?, 1, 0));
        self.emitted_return = true;
        Ok(())
    }

    fn finish(&mut self) -> Result<Function32> {
