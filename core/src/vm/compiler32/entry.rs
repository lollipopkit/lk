...
impl Compiler32 {
    pub fn compile_expr(expr: &Expr) -> Result<Function32> {
    ...
    pub fn compile_program(program: &Program) -> Result<Function32> {
        let mut compiler = Self::default();
        compiler.lower_program_statements(program)?;
        compiler.finish()
    }

    pub fn compile_module(program: &Program) -> Result<Module32> {
        ...
        S: AsRef<str>,
    {
        ...
        let mut module = Module32 {
            functions: vec![Function32::default(); function_names.len() + 1],
            natives,
        ...
        for stmt in &program.statements {
            ...
            } = stmt.as_ref()
            {
                let function_index = *function_names
                    .get(name)
                    .ok_or_else(|| anyhow!("Compiler32 missing function index for `{name}`"))?;
    ...
    pub fn compile_source(source: &str) -> Result<Function32> {
        let tokens = Tokenizer::tokenize(source)?;
        let program = StmtParser::new(&tokens).parse_program()?;
        Self::compile_program(&program)
    }

    pub fn compile_source_module(source: &str) -> Result<Module32> {
        Self::compile_source_module_with_natives(source, Vec::new())
    }

    pub fn compile_source_module_with_natives(source: &str, natives: Vec<NativeEntry32>) -> Result<Module32> {
        let tokens = Tokenizer::tokenize(source)?;
        let program = StmtParser::new(&tokens).parse_program()?;
