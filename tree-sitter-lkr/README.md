...
[Tree-sitter](https://tree-sitter.github.io/tree-sitter/) grammar for the [LKR](https://github.com/lollipopkit/lkr) programming language.

## Features

- Full grammar coverage for LKR: expressions, statements, types, patterns, concurrency primitives
- Incremental parsing via tree-sitter
- Supports all LKR language features: structs, closures, named parameters, match, ranges, ternary, nullish coalescing, optional chaining
...
```toml
[dependencies]
tree-sitter-lkr = { path = "path/to/tree-sitter-lkr" }
tree-sitter = "0.24"
```
...
const LKR = require('tree-sitter-lkr');

const parser = new Parser();
parser.setLanguage(LKR);
...
use tree_sitter_lkr::language;

let mut parser = Parser::new();
parser.set_language(&language()).expect("Error loading LKR grammar");
...
```bash
npx tree-sitter parse examples/fib.lkr
```
