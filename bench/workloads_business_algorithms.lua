...
local seed = os.time()
local workload_filter = os.getenv("LK_WORKLOAD_FILTER") or ""
...
local function should_run(name)
  return workload_filter == "" or workload_filter == name
end
...
if should_run("prime_trial_division") then
  t0 = os.clock()
  checksum = 0
...
if should_run("binary_search") then
  t0 = os.clock()
  checksum = 0
...
if should_run("two_sum_map") then
  t0 = os.clock()
  checksum = 0
...
if should_run("sliding_window_sum") then
  t0 = os.clock()
  checksum = 0
...
if should_run("matrix_3x3_multiply") then
  t0 = os.clock()
  checksum = 0
...
if should_run("stock_max_profit") then
  t0 = os.clock()
  checksum = 0
...
if should_run("histogram_group_count") then
  t0 = os.clock()
  checksum = 0
...
if should_run("string_key_hash") then
  t0 = os.clock()
  checksum = 0
...
if should_run("order_score_pipeline") then
  t0 = os.clock()
  checksum = 0
...
if should_run("log_parse_filter") then
  t0 = os.clock()
  checksum = 0
...
if should_run("cart_pricing_rules") then
  t0 = os.clock()
  checksum = 0
...
if should_run("route_permission_check") then
  t0 = os.clock()
  checksum = 0
...
if should_run("inventory_reorder") then
  t0 = os.clock()
  checksum = 0
...
if should_run("fraud_rule_scoring") then
  t0 = os.clock()
  checksum = 0
