...
4. `BrTrue/BrFalse` 支持已接入但默认不启用；`GetFieldK/SetFieldK` 已作为 const string field operand-shape opcode 落地。typed compare-test 已增加 target-pc control-flow fact、cold fallback split，并补充 `TestEqIntI` / `TestNeIntI` immediate literal shape。`ConcatN` 已作为 3+ part template/multi-register concat opcode 落地，并同步修复 AOT scalar block 中 dynamic string-map `GetFieldK` 与 optional nil branch 的 correctness。`Return0/Return1` 已作为常见返回路径 opcode 落地。下一步优先做 `GetI/SetI`，或继续 facts-confirmed compare-branch / hot-loop lowering。
