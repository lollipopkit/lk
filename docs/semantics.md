...
**这背后是一个架构问题,没有修**:宽度这件事类型检查器全都算过了,而编译器在
用一堆各自为政的旁表(`function_machine_returns`、`struct_field_machine_widths`、
`global_machine_widths`、`capture_machine_widths`、`local_struct_types`,现在又
多一张)按形状重新推一遍。每加一种"值可以从哪里出来"的形状,就多一个会漏的洞
——这四个就是这么来的。真正的修法是把类型检查器算出的类型交给编译器,那是一次
重新设计,不是一次补丁。
