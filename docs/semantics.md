...
**没做的**:让 `use { P } from "geo"` 真正把类型绑进来,从而裸 `P` 也能用。

方案已经查清(2026-08-06),而且**不需要新 opcode**:`core/src/stmt/struct_ctors.rs` 已经
给每个 `struct S` 生成 `fn S$new({…}) -> S`,`m.S { … }` 是解析期糖 `m.S$new(…)` ——
调用跑在定义方模块里,所以 scope、字段序、trait 分派全对。缺的只是把**裸名字**也接到
这条糖上,以及让 `use { P } from "m"` 绑定那个构造函数而不是拒绝。

哪一层做改写是唯一的取舍:编译器侧(需要被告知"哪些名字是导入类型",与它已经接收
`external_globals` 同类)代价可控;解析期一律改写(一种拼写、零特例)则把本地构造的
性能押在内联器上,`NewObject` 在热路径上,必须用 perf 门禁量。见任务表 #202 与 #74。
