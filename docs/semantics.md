...
然读得到,写 `m["len"]` 或 `m.len`(不带括号)。这条规矩此前只存在于实现的分
发顺序里 —— 没有写下来,也没有测试钉住,于是"我的 map 有个叫 `keys` 的键"这种
程序的行为取决于谁先被查。
...
**短路仍然是运行期语义**:`false && (1 / z == 1)` 不会求值右边、不会除零,这是
执行器做的事,和折不折无关。恒定条件的分支消除属于优化,归类型检查之后的 VM
