...
## 已知分歧:`try` 体里调用的函数改了容器,原生丢掉这次修改(2026-07-31 记,未修)

```lk
fn note(xs: List<Int>, n: Int) -> Int { xs.push(n); return n; }
let xs: List<Int> = [];
println(try { "${note(xs, 2)}" } catch e { "c" });
println("B${xs.len()}");        // VM: B1    native: B0
```

**这是错答案,不是回落** —— 程序照跑,打出一个看着合理的长度。

根因不在 `try`:`try` 体让 `note` 的形参被拓宽成 `Dyn`,而**类型化列表是唯一一
个装箱时会重建容器的载体**(`list_h.i64_to_dyn` 逐元素装箱成新表)。集合、
bytes、窗口、dyn 列表和**全部五种类型化 map** 都是原地打标签装箱 —— `DYN_RAW`
的注释早就把规矩写下了:"装箱不得重新表示一个容器"。类型化 map 正是因为违反它
才被修过(重建后的迭代序和 VM 不一样)。类型化列表是同一条规矩最后没补的那一格。

正确修法就是那条已有先例的:**给类型化列表各自的 tag,原地装箱**(照
`DYN_TMAP_BASE` 那次做)。

试过一个便宜的挡法 —— 在"实参装箱"处拒绝降低,让程序回落拿正确答案 —— 但要判
"这份拷贝会不会被看见"就得知道被调方会不会改容器,而调用图要穿过接受回调的内建
(`xs.reduce([], |a, x| f(a, x))`),那正是降低之前答不出来的问题。保守到能挡住
就会把 `insertion_sort` 这类纯多态帮手一起挡掉(AOT 覆盖 60/60 → 58/60)。**没
有落地半成品**:与其换一个不精确的挡法,不如把分歧原样记在这里,按原地装箱去修。
