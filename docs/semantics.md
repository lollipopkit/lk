...
| `println(map)` | hash 迭代序 | map display 顺序 = 底层 hash map 迭代序,**跨运行稳定但不可移植**(依赖 hasher+增长历史)。2026-07-30 更正:**字符串键的 map native 侧照样显示** —— `vm_mirror` 就是为"两边共享这个序"写的。仍然拒绝的只有整数键,理由见下 |
...
现在两边都显示容器,差分语料补了这条。留在原生子集外的只剩 **Set**,以及
**整数键的 map**(见下)—— 字符串键的 map 已经进来了。
...
现在字符串键的三种都显示。**整数键的仍然拒绝**,而且理由是新查出来的,不是老裁决:
VM 对非字符串键不做第二阶段(`typed_map_from_entries` 直接返回 `Mixed`),
lkrt 的 `lit_finish_i64_*` 却又 rehash 了一遍,两边的顺序真的不一样。
