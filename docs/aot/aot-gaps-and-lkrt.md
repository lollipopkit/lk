...
**类型化容器仍然不行**,而且不是缺一行表:它的装箱是**逐元素转换**(`list_h::i64_to_dyn`
会建第二个列表),往返会拿回一个副本,体里对原句柄的写就丢了;而把类型化句柄直接打上
`DYN_LIST` 比"错"更糟 —— 一个 `Vec<i64>` 被当成 `Vec<LkDyn>` 读是内存安全 bug。它们需要
的是**保身份的 cell**(裸句柄槽,不是装箱槽)。
