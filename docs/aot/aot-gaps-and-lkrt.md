...
不止空字面量的猜测,也不止 nil/Bool。任何**跨种类的键**写进类型化 map 载体都是错答,
...
真实代价只有一处:`examples/syntax/closure_value.lk` 不再整体原生化。它里面
`try { m[f] = 1; }` 故意拿一个闭包当键,期待运行期按解释器的措辞拒绝 —— 那一条
**降级是对的**(两端都 raise)。但闭包这个事实活在 `ssa.closure_values` 里,而它是
按函数的 SSA 状态,过不了 `try` region 的边界(body 是独立函数,`f` 变成参数)。
所以精确的守卫需要把"这是闭包"这件事跨 region 传下去,或者干脆做下面这件事。
