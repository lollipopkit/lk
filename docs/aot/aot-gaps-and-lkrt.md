...
**嵌套不可回避,所以运行时表也不可回避。** `obj_mark` 已经在给 arena 对象打
tid;要补的是:把每个类型的 name + 字段名(声明序)作为静态表发进目标文件,
一个 lkrt helper 查这个标记来渲染,并让 `dyn` 的 `display`/`display_quoted`
在遇到已标记对象时走它 —— 这样嵌套自然递归。
