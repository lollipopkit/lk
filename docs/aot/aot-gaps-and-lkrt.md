...
下一层的结构性问题还在:结构体实例是个字符串键哈希表,一个字段 ~125ns,而列表的一个元素 ~16ns。
按声明序排列的**记录**表示能把前者拉到后者附近(字段下标编译期已知,`str_dyn_get_at` 那条已经
在读的一侧走通了),代价是构造、读写、display、相等、迭代、spread 都要跟着改,而且声明不可达的
结构体(跨模块、宿主建的)还得保留 map 形式。
