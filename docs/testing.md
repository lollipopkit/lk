...
| VM vs native sweep | `SWEEP_REQUIRE="identical=78 diverged=1" bash scripts/vm_native_sweep.sh` | check.yml | Every example and bench program under both executors. The gap between "does it lower" and "does this pinned case agree": lowers fine, wrong answer, no corpus case with that shape. |
