...
1. **基于 Instr32 重建**：LLVM 后端迁移完成后必须消费 `Module32` / `Instr32`，不再接入旧 bytecode、LKB bundle 或旧 VM 执行器。
2. **块级翻译**：`lk_core::llvm::LlvmBackend` 逐基本块遍历 Instr32 指令，输出文本 LLVM IR（统一使用 `i64`、不透明指针），保持原始控制流结构。
3. **可选优化**：后端可调用 `opt`（来源为 `llvm-tools-preview` 组件或 `LK_LLVM_OPT` 环境变量指定路径）生成优化版 IR，与原始 IR 并存。

```
Program (AST) ──Compiler32──▶ Module32/Instr32 ──LLVM translator──▶ module.ll
                                                                             │
...
运行时接口：当 IR 需要字符串常量、全局读写、集合构造或 `ToStr` / `Call` 等操作时，会声明 `lk_rt_*` helper。LLVM 后端当前在 Instr32 VM 迁移期间禁用，重新启用时应基于 Instr32 module/runtime callable，而不是旧 bytecode、LKB bundle 或旧 VM 执行器。

这些符号由 `core/src/llvm/runtime.rs` 提供：模块内部维护一个轻量 `VmContext`、字符串 interner 以及句柄表，将 LLVM 传入的 64-bit 编码值翻译回 `Val`。迁移期间，旧 bytecode trampoline 和 `lk_rt_run_bytecode` 入口已删除；重新启用 AOT 时必须生成 Instr32/RuntimeCallable 兼容入口。
