...
- [x] CLI 集成：`lk compile llvm FILE` 输出 `.ll`，可选保留 `.unopt.ll`。
- [x] CLI 增加 `lk compile exe FILE`，串联 `llc` 与系统链接器生成原生可执行文件。
...
2. **块级翻译**：`lk_core::llvm::LlvmBackend` 逐基本块遍历 VM 指令，输出文本 LLVM IR（统一使用 `i64`、不透明指针），保持原始控制流结构。
3. **可选优化**：后端可调用 `opt`（来源为 `llvm-tools-preview` 组件或 `LK_LLVM_OPT` 环境变量指定路径）生成优化版 IR，与原始 IR 并存。
...
运行时接口：当 IR 需要字符串常量、全局读写、集合构造或 `ToStr` / `Call` 等操作时，会自动声明 `declare i64 @lk_rt_intern_string(ptr, i64)`、`declare i64 @lk_rt_to_string(i64)`、`declare i64 @lk_rt_load_global(i64)`、`declare void @lk_rt_define_global(i64, i64)`、`declare i64 @lk_rt_build_list(ptr, i64)`、`declare i64 @lk_rt_call(i64, ptr, i64, i64)`、`declare i64 @lk_rt_in(i64, i64)`、`declare i64 @lk_rt_list_slice(i64, i64)` 等 helper，并在入口块中生成对应的调用。自 2025-10 起，main stub 还会注入 `lk_rt_begin_session` / `lk_rt_register_search_path` / `lk_rt_register_bundled_module` / `lk_rt_register_imports` / `lk_rt_apply_imports`，用于在原生可执行文件中重放模块导入、预注册打包的 LKB 模块以及初始化标准库。

这些符号由 `core/src/llvm/runtime.rs` 提供：模块内部维护一个轻量 `VmContext`、字符串 interner 以及句柄表，将 LLVM 传入的 64-bit 编码值翻译回 `Val`。`lk compile exe` 会在编译阶段遍历 AST，并优先把可 native lowering 的 package module 顶层函数编译为 LLVM 函数，通过 `lk_rt_register_native_module_function` 注册到 AOT runtime；入口里的 `module.func(...)` 会被降为 `lk_rt_call_method`，避免把 bundled LKB、`execute_imports` 和 VM 执行器塞进这类可执行文件。暂不支持 native lowering 的文件/package import 仍会回退到 bytecode trampoline 或 bundled-module 路径，以保留语义正确性。

为了满足链接需求，CLI 会自动构建并链接 release 版 `liblk_aot_runtime.a`。该专用 staticlib 导出 AOT 所需的 `lk_rt_*` helper，并使用 `lk-core/aot-minimal-runtime` feature 禁用 VM-only core builtins。AOT runtime 默认只注册 `print` / `println` / `panic` 等核心全局，并根据入口文件实际 import 的 stdlib 模块按需注册模块，避免把完整 stdlib 都拉进每个可执行文件。需要本地 `cargo`、LLVM 工具链（`llc` 等）以及静态链接器均可用。链接 Apple 目标时，CLI 会传入 `-Wl,-dead_strip`，并在本机目标链接成功后自动调用 `strip` 压缩符号表，降低产物体积与冷启动开销。

Workspace release profile 面向 AOT 体积优化：启用 `lto = true`、`codegen-units = 1`、`opt-level = "z"` 与 `panic = "abort"`，因此首次构建 release runtime 会更慢，但后续会复用 `target/lk-native/release` 缓存。

发布二进制版 CLI 时，务必把上述静态库一起打包（例如 `bin/lk`、`lib/liblk_aot_runtime.a`）。运行时会首先尝试从 `LK_RUNTIME_LIB_DIR` 指定的目录、或可执行文件旁的 `lib/`、`lib/<target-triple>/` 及其 `release` / `debug` 子目录复用该库，只有在找不到时才回退到 `cargo build`。这样即便目标机器上没有完整的 Rust 工具链，也能执行 `lk compile exe`。
...
- **缺点**：
  ...
  - 目前的 `lk_rt_*` API 基于统一字宽，需要引入新的 FFI 层做拆装箱。
...
- 后续工作指引：
  ...
  3. 调整所有 `lk_rt_*` helper 的签名与实现，使其接受/返回新的 `Value` 结构，并在链接文档中同步 ABI 说明。
...
```
lk compile llvm [path/to/file.lk] [--opt-level {O0|O1|O2|O3}] [--skip-opt] [--target-triple TRIPLE]
lk compile exe [path/to/file.lk] [--opt-level {O0|O1|O2|O3}] [--skip-opt] [--target-triple TRIPLE] [--output PATH]
```

- 省略 `path/to/file.lk` 时，CLI 使用当前目录的 `main.lk`；若当前目录存在 package `Lk.toml`，则使用 `src/main.lk`；若当前目录是 workspace root 且只有一个成员 package 包含 `src/main.lk`，则使用该 app 入口。多个 app 入口时仍需显式传入文件。
...
- `--output`：仅对 `compile exe` 生效，指定最终可执行文件输出路径。默认路径按目标平台选择：Apple 目标为 `<源文件名>`，Windows 目标为 `<源文件名>.exe`，Linux / ELF 目标为 `<源文件名>.elf`，未知目标为 `<源文件名>.out`。

工具查找顺序：

| 功能 | 环境变量优先级 | 备用来源 |
| ---- | -------------- | -------- |
| `opt` | `LK_LLVM_OPT` | `llvm-tools-preview` / 系统 `PATH` |
| `llc` | `LK_LLVM_LLC` | `llvm-tools-preview` / 系统 `PATH` |
| 链接器 | `LK_CC` → `CC` → `cc` | 系统 `PATH` |

`compile exe` 会先调用 LLVM 后端生成 `.ll`（同样在需要时保留 `.unopt.ll`），然后执行：

1. `llc -filetype=obj` → 生成目标文件 `<name>.o`（尊重 `--target-triple`）。
2. `cc` / `clang` / `LK_CC` → 链接生成原生可执行文件，可通过 `--output` 改写目标路径。
3. Apple 目标额外传入 `-Wl,-dead_strip`，本机目标默认执行 `strip`（可用 `LK_STRIP` 指定工具路径）；显式交叉编译目标不会调用宿主 `strip`。

生成的 IR 会自动附加一个最小化的 `main` Stub（调用 `@lk_entry` 并忽略返回值），方便直接链接成可执行文件。

示例：

```
$ lk compile examples/arith.lk --emit llvm --opt-level O1
Emitted LLVM IR to examples/arith.ll (optimised, opt-level O1)
Preserved unoptimised IR at examples/arith.unopt.ll

$ lk compile exe examples/arith.lk --target-triple x86_64-unknown-linux-gnu
Emitted native executable to examples/arith.elf (backend O2, runtime release, not stripped, LLVM IR at examples/arith.ll)
```
...
- 单元测试：`cargo test -p lk-core llvm::tests`，覆盖算术、分支、短路逻辑、空合并与判空跳转。
- CLI 冒烟：`cargo run -p lk-cli -- compile llvm path/to/file.lk --skip-opt`，在缺少 LLVM 工具链的环境中验证端到端流程。
...
1. **运行时符号实现**
   - [x] 在 `core/src/llvm/` 下新增 `runtime.rs`，实现 `lk_rt_in`、`lk_rt_list_slice` 等 helper，并明确 64-bit 值编码（立即数、句柄、标记位）。
   - [x] 以 `#[no_mangle] extern "C"` 暴露接口，复用 `VmContext` 与 stdlib 逻辑，确保 AOT 链接阶段符号可解析。
2. **链接链路完善**
   - [x] 新增 `lk-aot-runtime` staticlib，`lk compile exe` 自动链接 release AOT runtime。
   - [x] 在 `docs/llvm/linker.md` 同步记录最终链接命令、依赖工具与环境变量约定。
3. **剩余指令覆盖**
