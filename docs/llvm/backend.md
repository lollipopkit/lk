...
这些符号由 `core/src/llvm/runtime.rs` 提供：模块内部维护一个轻量 `VmContext`、字符串 interner 以及句柄表，将 LLVM 传入的 64-bit 编码值翻译回 `Val`。迁移期间，bundled LKB 注册和 `lk_rt_run_bytecode` 只保留为拒绝旧入口的边界，不作为可执行路径。
