...
- [x] CLI 增加 `lk compile exe FILE`，串联 `llc` 与系统链接器生成 ELF 可执行文件。
...
为了满足链接需求，CLI 会自动构建并链接 `liblk_core.a` 与 `liblk_stdlib.a`：前者导出所有 `lk_rt_*` helper，后者提供 `lk_stdlib_register_*` 桥接函数以注入内置模块/全局。需要本地 `cargo`、LLVM 工具链（`llc` 等）以及静态链接器均可用。
...
2. `cc` / `clang` / `LK_CC` → 链接生成 ELF，可通过 `--output` 改写目标路径。
...
2. **链接链路完善**
   - [ ] 在 `Cargo.toml` 启用 `staticlib` / `cdylib` 输出，`lk compile exe` 自动链接 runtime。
