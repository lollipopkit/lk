...
这些符号由 `core/src/llvm/runtime.rs` 提供：模块内部维护一个 `VmContext`、字符串 interner 以及句柄表，将 LLVM 传入的 64-bit 编码值翻译回 `Val` 并重用既有 VM 语义（列表/字典构造、`ToIter` 等）。`lk compile exe` 会在编译阶段遍历 AST，借助 `ModuleBundler` 提前编译文件类导入，序列化导入语句，并将二进制 LKB 片段作为 LLVM 全局常量嵌入；运行时由上述 helper 逐个注册，从而在没有 VM 的情况下也能复现模块解析流程和 `execute_imports` 语义。

为了满足链接需求，CLI 会自动构建并链接 release 版 `liblk_core.a` 与 `liblk_stdlib.a`：前者导出所有 `lk_rt_*` helper，后者提供 `lk_stdlib_register_*` 桥接函数以注入内置模块/全局。需要本地 `cargo`、LLVM 工具链（`llc` 等）以及静态链接器均可用。链接本机目标时，CLI 会在链接成功后自动调用 `strip` 压缩符号表，降低产物体积与冷启动开销。

发布二进制版 CLI 时，务必把上述静态库一起打包（例如 `bin/lk`、`lib/liblk_core.a`、`lib/liblk_stdlib.a`）。运行时会首先尝试从 `LK_RUNTIME_LIB_DIR` 指定的目录、或可执行文件旁的 `lib/`、`lib/<target-triple>/` 及其 `release` / `debug` 子目录复用这些库，只有在找不到时才回退到 `cargo build`。这样即便目标机器上没有完整的 Rust 工具链，也能执行 `lk compile exe`。
...
3. 本机目标默认执行 `strip`（可用 `LK_STRIP` 指定工具路径）；显式交叉编译目标不会调用宿主 `strip`。
...
2. **链接链路完善**
   - [x] 在 `Cargo.toml` 启用 `staticlib` 输出，`lk compile exe` 自动链接 release runtime。
