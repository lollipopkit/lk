...
- **"推倒重写"应执行为四件事，而非从零**：(1) 分层去全局状态（修复并发/耦合）；(2) 引入 `pcall`/`error`/`try` 可恢复错误模型；(3) AOT 先做"字节码嵌入"这一 100% 覆盖、语义平凡一致的形态，再做逐函数回退 VM 的混合模式（彻底消除"全有或全无"）；(4) 砍掉签名注册表包管理器与重型 LSP，换成 git+lockfile 与 tree-sitter。
...
5. **范围收敛是 solo 维护成败的关键**：签名注册表（HMAC/Ed25519 keyring、`lk pkg serve`、publish/yank）与重型 LSP（macro-origin hover/goto-definition/semantic tokens）对一个 14-star 单人项目属过度工程，应替换为 git+lockfile（Deno/Go 模式）与 tree-sitter。
...
**非目标**：不做运行时 JIT（仅 AOT+解释）；不保证向后兼容；`.lkm` 不作对外分发格式；v1.0 不交付完整语义 LSP；v1.0 不做真正裸机（core-only 无 allocator），最小目标为 WASM + 带 allocator 的 MCU。
...
│  L6  lsp-lite / tree-sitter  (std, 可选)                  │
├─────────────────────────────────────────────────────────┤
...
### 九、工具链与生态（砍范围保 solo，修复问题 7）

- **CLI**：`lk`（REPL）、`lk FILE`、`lk check`、`lk compile`（Tier 0/1）、`lk fmt`。
- **tree-sitter 语法（保留）优先于完整 LSP**：tree-sitter 提供快速、容错（含语法错误时产部分解析树）的高亮 + 结构查询，Zed/Neovim/Helix 开箱即用，成本远低于完整 LSP。
- **延后完整 LSP**：完整语义 LSP 需一套 latency-sensitive 的编译器前端重实现（如 rust-analyzer），对 solo 是巨大负担。v1.0 只交付 tree-sitter + `lk-lsp-lite`（`lk check` 诊断经 LSP 透传 + tree-sitter 符号高亮）。现状重型 LSP（macro-origin hover/goto-definition/semantic tokens）**建议冻结/降级**。
...
- `bare`/`alloc`/`full` 三 profile 打通；WASM + 一类 MCU 冒烟；`lk fmt`；tree-sitter 完善；重型 LSP 降为 `lk-lsp-lite`；包管理缩减为 git+lockfile。
...
| 7 | 范围过大 | M5：砍签名注册表→git+lockfile；重型 LSP→tree-sitter+lite |
...
4. **无情砍范围**：签名注册表包管理器与重型 LSP 是 solo 维护最大拖累，换成 git+lockfile 与 tree-sitter。
...
- **部分现状实现细节来自仓库内 markdown 文档（README/plan/progress/handoff/docs）而非直接源码**：GitHub 目录页与 raw `.rs` 文件在调研时无法直接抓取。以下事项建议作者直接在源码核对：(a) `Value` 枚举确切名称与变体、是否有任何 NaN-boxing（文档暗示 tagged union + Arc 堆载荷，无 NaN-boxing）；(b) VM 错误枚举确切类型名（如 `VmError`/`RuntimeError`）；(c) `core` VM crate 是否含 `once_cell`/`lazy_static`（仅在 `lkrt` 确认了 `thread_local! RefCell`）。
...
- **砍 LSP/签名包管理是范围取舍判断**，非技术必然。若项目后续获得更多贡献者或商业需求，这些能力可按需恢复；本规划立场是"对当前 solo + 14-star 规模属过度工程"。