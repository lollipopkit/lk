...
已完成的特定优化保留在代码中，但不再作为当前主线继续扩张：

- tiny call：GCD、binary search、prime trial division。
- packed typed dispatch：integer arithmetic/compare/move、string predicate branch、map upsert、
  list int access feeding arithmetic。
- AOT/map/list/string 若干 helper lowering。

这些历史工作说明局部优化有效，但后续新增工作必须转向通用架构。下一步从
`PerformanceFacts` 开始，把现有事实统一起来，再让 typed lowering、call frame、
`Val` ownership 和 VM/AOT lowering 逐步消费同一套事实。
