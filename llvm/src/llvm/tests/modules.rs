...
fn llvm_backend_lowers_static_module_return_display_without_artifact_shell() {
    ...
    let module = Compiler::compile_module_with_natives_and_globals(&program, Vec::new(), ["iter"]).expect("module");
...
fn llvm_backend_lowers_dynamic_bool_list_module_methods_without_artifact_shell() {
    ...
    assert!(!artifact.module.ir.contains("@lk_module_json"));
    assert!(!artifact.module.ir.contains("lk_rt_run_module_json"));
    assert!(artifact.module.ir.contains("@lkrt_list_i64_contains"));
    assert!(artifact.module.ir.contains("@lkrt_list_i64_index_of"));
    assert!(artifact.module.ir.contains("call void @lkrt_list_i64_reverse"));
    assert!(artifact.module.ir.contains("call void @lkrt_list_i64_sort"));
    assert!(artifact.module.ir.contains("call void @lkrt_list_i64_concat"));
    assert!(artifact.module.ir.contains("@lkrt_list_i64_pop"));
...
fn llvm_backend_lowers_dynamic_bool_list_module_mutators_without_artifact_shell() {
    ...
    assert!(!artifact.module.ir.contains("@lk_module_json"));
    assert!(!artifact.module.ir.contains("lk_rt_run_module_json"));
    assert!(artifact.module.ir.contains("@lkrt_list_i64_push"));
    assert!(artifact.module.ir.contains("@lkrt_list_i64_slice_range"));
    assert!(artifact.module.ir.contains("@lkrt_list_i64_insert"));
    assert!(artifact.module.ir.contains("@lkrt_list_i64_remove_at"));
    assert!(artifact.module.ir.contains("@lkrt_list_i64_set"));
}
...
fn llvm_backend_lowers_dynamic_i64_map_has_delete_without_artifact_shell() {
    for source in [
        ...
        let program = StmtParser::new(&tokens).parse_program().expect("program");
        let module =
            Compiler::compile_module_with_natives_and_globals(&program, Vec::new(), ["__lk_call_method", "map"])
                .expect("compile module");
        let module = ModuleArtifact::new(Vec::new(), &module).expect("artifact");

        let artifact =
            compile_module_artifact_to_llvm(&module, super::legacy_fallback_options()).expect("llvm artifact");
...
fn llvm_backend_lowers_dynamic_string_map_has_delete_without_artifact_shell() {
    for source in [
        ...
        assert!(!artifact.module.ir.contains("@lk_module_json"));
        assert!(!artifact.module.ir.contains("lk_rt_run_module_json"));
        // has/delete are lowered to lkrt typed helpers (contains + monomorphized
        // `_delete`) rather than the old hand-written inline loops.
        assert!(artifact.module.ir.contains("call i64 @lkrt_map_str_contains("));
        assert!(artifact.module.ir.contains("_delete(ptr "));
        assert!(artifact.module.ir.contains("ret.arg.map."));
    }
}
...
fn llvm_backend_lowers_dynamic_string_string_map_missing_get_without_artifact_shell() {
    for expr in ["map.get(without, \"k1\")", "without[\"k1\"]", "without.get(\"k1\")"] {
        ...
        let artifact = compile_module_artifact_to_llvm(&module, super::legacy_fallback_options())
...
fn llvm_backend_lowers_dynamic_i64_list_module_methods_without_artifact_shell() {
    ...
    assert!(artifact.module.ir.contains("@lkrt_list_i64_contains"));
    assert!(artifact.module.ir.contains("@lkrt_list_i64_index_of"));
    assert!(artifact.module.ir.contains("@lkrt_list_i64_reverse"));
    assert!(artifact.module.ir.contains("@lkrt_list_i64_pop"));
}
...
return [
  ...
  string.is_empty("")
    ...
    let program = StmtParser::new(&tokens).parse_program().expect("program");
    let module =
        Compiler::compile_module_with_natives_and_globals(&program, Vec::new(), ["string", "__lk_call_method"])
            .expect("module");
    let module = ModuleArtifact::new(Vec::new(), &module).expect("module artifact");
...
return [
  ...
  map.get({"a": 1}, "z")
    ...
    .expect("tokens");
    let program = StmtParser::new(&tokens).parse_program().expect("program");
    let module = Compiler::compile_module_with_natives_and_globals(&program, Vec::new(), ["map", "__lk_call_method"])
        .expect("module");
    let module = ModuleArtifact::new(Vec::new(), &module).expect("module artifact");
...
return [
  ...
  iter.collect(iter.range(3))
    ...
    .expect("tokens");
    let program = StmtParser::new(&tokens).parse_program().expect("program");
    let module = Compiler::compile_module_with_natives_and_globals(&program, Vec::new(), ["iter", "__lk_call_method"])
        .expect("module");
    let module = ModuleArtifact::new(Vec::new(), &module).expect("module artifact");
    let artifact = compile_module_artifact_to_llvm(&module, super::legacy_fallback_options()).expect("llvm artifact");
