...
/// (`hybrid` per `LK_AOT_HYBRID`, so a non-lowering helper bridges to the VM
/// instead of failing the module), validate, and emit a native object. The outer
/// `Result` is a genuine internal failure (validation/codegen bug), never a user
/// error; the inner result is:
...
pub fn compile_artifact_to_clif_object(
    ...
    // Match the string-IR path's default (`LK_AOT_HYBRID` on unless `=0`): a
    // reachable helper that does not lower natively is bridged to the VM.
