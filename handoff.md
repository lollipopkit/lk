...
- **C 进行中:v2 桥接返回值**(分支 feat/hybrid-v2-return-bridge,stacked 于 PR-A):
  - ✅ **C1-C5 完成并全验证**:C1 core keep-state(ModuleFunctionOutcome,修
    ...
    **lkrt display_into 补 DYN_MAP 臂**(VM 格式;此前 raise→abort 是更差分歧;
    ...
    三差分全绿。
  - **C6 待做(raise 进 native try)**:桥 Err 时经 wrapper 注入的 lkrt_rt_raise_*
    指针 longjmp 进最近 native try(先 drop 全部 guard);**关键调查点**:raise
    的一等值跨 anyhow 边界如何保真(pending_raise_root?仅文本会让 catch e 看到
    string 而非原值);lkrt no-handler 分支加打印 CURRENT_ERROR。
  - **C7 待做(fuzz+文档)**:hybrid 帮手已退化(try/catch 现可原生 lower)→
    body 换动态格式串 println + compile stderr 断言 "Tier 1 hybrid";加返回值
    被消费/容器返回形状;tier1-hybrid.md v2 章节(状态行同时修:现仍写 not
