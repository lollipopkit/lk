...
- **M2 后端基础设施大体就绪**(codebase 比 plan 假设成熟):`Opcode::Raise`/`TryBegin`/`TryEnd`、
  `ErrorHandler`(catch_reg/pc/frame_base)、`ErrorVal{message,trace}`(GC-rooted 结构化错误)。
  缺口:Raise 只载字符串(需扩任意 RuntimeVal)、**无语言层 pcall/error/try 面**(现用户级错误=nil+`??`)、
  fatal guard 走 abort 需改可捕获。→ M2.1 落地=pcall/error 内建 + 扩 Raise/ErrorVal 载值 + 桥接现有 handler。
- **M1.1 conformance**:examples_differential(llvm-gated,VM==AOT)+ M1.2(VM source==bytecode)已构成
  部分 golden;可补非-llvm 纯 VM golden。

## 剩余大步(按 phase)
- **M0.1** 关键路径:选项 A(trait 反转 callable,断 val→vm)——深改 call 热路径+GC,多小时。
- M0.7/8 no_std 化 core(tokio+102 use std feature-gate,多天)。
- M2(pcall 语言面+stackless+fuel)、M3(embed API+C ABI)、M4(AOT Tier 0/1)、M5(no-std profile+MCU+fmt+pkg)。
- 用户指令:**允许短期回归、fix-forward**;每步 push。
