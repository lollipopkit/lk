...
- **Hover Information**: Shows LK and stdlib signatures in Markdown code fences, renders doc comments as Markdown, and includes type/implementation navigation links
...
- Type names in signatures link to LK-defined `struct`, `trait`, or `type` declarations in the same document. Built-in LK runtime types such as `Int`, `String`, `List`, `Number`, and `Bytes` link to the language specification when a matching section is available.
- Stdlib hover uses metadata generated by `lk_stdlib_common::StdlibModule` and function-level `#[stdlib_export]`; signatures render in a fenced `lk` code block, docs render as Markdown, and Rust-backed exports include a `Go to implementation` link.
