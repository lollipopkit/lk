...
fn test_file_import_diagnostic_resolves_relative_to_current_file_dir() {
    ...
    base.push(format!("lk-lsp-import-test-{}", std::process::id()));
    ...
    let fib_path = nested_import_dir.join("fib.lk");
...
"#,
    ...
    .unwrap();
    fs::write(dep_src.join("mod.lk"), "fn answer() { return 42; }\n").unwrap();
...
fn test_example_workspace_imports_are_resolved() {
    ...
    let code = fs::read_to_string(&main_path).expect("read example workspace main.lk");
